...
struct PpuStimulus : public sc_module {
  ...
  const u8 tile_data[16 * 8] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  // tile0 = black
  ...
  PpuStimulus(u8* memory, sc_module_name name) : sc_module(name), targ_socket("targ_socket"), memory(memory) {
...
struct Top : public sc_module {
  ...
  explicit Top(sc_module_name name [[maybe_unused]])
      : test_stimulus(memory, "test_stimulus"), test_ppu("test_ppu", PpuArgs{options.headless, options.fps_cap}) {
